...
-- Set default schema
SET search_path = price_tracker, public;
...
CREATE TABLE IF NOT EXISTS users (
    ...
    provider VARCHAR(50) DEFAULT 'local', -- local, google, microsoft, apple
...
CREATE TABLE IF NOT EXISTS user_products (
    ...
    max_price DECIMAL(12,2),    -- alert when price goes above this
    is_active BOOLEAN DEFAULT true,
...
CREATE TABLE IF NOT EXISTS price_history (
    ...
    stock_status VARCHAR(50), -- in_stock, out_of_stock, limited_stock
...
CREATE TABLE IF NOT EXISTS notifications (
    ...
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    product_id UUID NOT NULL REFERENCES products(id) ON DELETE CASCADE,
    type VARCHAR(50) NOT NULL, -- price_drop, price_increase, back_in_stock
...
INSERT INTO users (email, first_name, last_name, is_verified) VALUES
                                                                  ('developer@example.com', 'Dev', 'User', true),
                                                                  ('test@example.com', 'Test', 'User', true)
    ON CONFLICT (email) DO NOTHING;

-- Create a sample product for testing
INSERT INTO products (url, title, domain, selector_config) VALUES
    ('https://didongviet.vn/may-tinh-bang/ipad-air-m3-11-inch-128gb-wifi.html',
     'iPad Air M3 11 inch 128GB WiFi',
     'didongviet.vn',
     '{"price_selector": ".price-current", "title_selector": "h1.product-title", "image_selector": ".product-image img"}'::jsonb)
    ON CONFLICT (url) DO NOTHING;

-- Grant permissions (if needed)
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA price_tracker TO devuser;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA price_tracker TO devuser;